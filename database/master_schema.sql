...
-- MASTER DATABASE SCHEMA & SEED DATA MIGRATION (SUPABASE / POSTGRESQL 15)
-- ==============================================================================
...
-- Suppliers Seed Data
INSERT INTO suppliers (id, company_name, contact_person, email, phone, address, payment_terms, rating)
VALUES 
  ('a1111111-1111-1111-1111-111111111111', 'TechMart Microelectronics India', 'Rajesh Kumar', 'procurement@techmart.in', '+91 98765 43210', '{"city":"Bengaluru","country":"India"}'::jsonb, 'Net 30', 4.85),
  ('a2222222-2222-2222-2222-222222222222', 'Apex Components Corp', 'David Miller', 'orders@apexcomp.com', '+1 408 555 0199', '{"city":"San Jose","country":"USA"}'::jsonb, 'Net 15', 4.92)
ON CONFLICT (email) DO NOTHING;

-- Warehouses Seed Data
INSERT INTO warehouses (id, name, city, address, total_zones)
VALUES 
  ('w1111111-1111-1111-1111-111111111111', 'Central Megahub Alpha', 'Chicago, IL', '100 Logistics Parkway, Chicago, IL', 5)
ON CONFLICT (id) DO NOTHING;

-- Products Seed Data
INSERT INTO products (id, sku, barcode, name, category, brand, purchase_price, selling_price, reorder_threshold, reorder_quantity, supplier_id)
VALUES 
  ('p1111111-1111-1111-1111-111111111111', 'SKU-LAPT-001', '8901234567890', 'ProBook Enterprise Laptop 15" i7 32GB', 'Electronics', 'ProBook Tech', 850.00, 1299.99, 10, 50, 'a1111111-1111-1111-1111-111111111111'),
  ('p2222222-2222-2222-2222-222222222222', 'SKU-MONI-4K27', '8901234567891', 'UltraClear 27" 4K IPS HDR Monitor', 'Electronics', 'UltraClear', 320.00, 499.00, 15, 40, 'a2222222-2222-2222-2222-222222222222')
ON CONFLICT (sku) DO NOTHING;

-- Customers Seed Data
INSERT INTO customers (id, name, email, phone, company, tier, total_orders, total_spend)
VALUES 
  ('c1111111-1111-1111-1111-111111111111', 'Vanguard Aerospace LLC', 'procurement@vanguardaerospace.com', '+1 206 555 9811', 'Vanguard Aerospace', 'vip', 42, 185400.00)
ON CONFLICT (email) DO NOTHING;
