...
export const ARIAChatConsole: React.FC = () => {
  ...
  return (
    <div
      role="region"
      aria-label="ARIA Operational Query Terminal"
      className="flex flex-col h-[520px] rounded-2xl border border-slate-200 dark:border-purple-900/60 bg-white dark:bg-[#080C14] text-slate-900 dark:text-slate-100 shadow-sm dark:shadow-2xl overflow-hidden font-sans"
    >
      {/* Terminal Title Bar */}
      <div className="flex items-center justify-between border-b border-slate-100 dark:border-purple-900/40 bg-slate-50 dark:bg-[#0D131F] px-5 py-3.5">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xs font-bold uppercase tracking-wider text-purple-700 dark:text-purple-300">
      ...
      {/* Terminal Output Window — Telemetry Feed */}
      <div
        role="log"
        aria-live="polite"
        aria-label="Telemetry Feed Log"
        className="flex-1 overflow-y-auto p-4 space-y-4 text-xs scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-800"
      >
        {chatHistory.map((msg) => (
          <div
            key={msg.id}
      ...
      <div className="flex items-center gap-2 overflow-x-auto px-4 py-2 bg-slate-50 dark:bg-[#0D131F] border-t border-slate-100 dark:border-purple-900/30 text-[11px]">
        <span className="text-[10px] text-slate-500 uppercase font-semibold shrink-0">Sample Queries:</span>
        <button
          type="button"
          aria-label="Ask which SKUs are at stockout risk"
          onClick={() => handleSend('Which SKUs are at stockout risk?')}
          className="px-2.5 py-1 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-purple-700 dark:text-purple-300 hover:border-purple-500 transition shrink-0 font-medium focus:outline-none focus:ring-2 focus:ring-purple-500"
        >
          Which SKUs are at stockout risk?
        </button>
        <button
          type="button"
          aria-label="Show active wave picking SLA bottlenecks"
      ...
      <form
        ...
        aria-label="Submit Operational Query"
        className="flex items-center border-t border-slate-100 dark:border-purple-900/40 bg-white dark:bg-[#0D131F] p-3"
      >
        <span className="text-purple-600 dark:text-purple-400 mr-2 font-bold" aria-hidden="true">&gt;</span>
        <input
          type="text"
          value={inputQuery}
          onChange={(e) => setInputQuery(e.target.value)}
          aria-label="Ask ARIA terminal input"
        ...
        <button
          type="submit"
          aria-label="Send Query to ARIA"
          disabled={loading}
          className="ml-2 rounded-xl bg-purple-600 hover:bg-purple-700 p-2.5 text-white transition disabled:opacity-50 shadow-xs focus:outline-none focus:ring-2 focus:ring-purple-500"
        >
