...
export const PickingRouteMap: React.FC<PickingRouteMapProps> = ({ items, activeBinCode }) => {
  ...
  return (
    <div className="flex flex-col gap-4 rounded-xl border border-slate-800 bg-[#0D131F] p-4 text-slate-100 shadow-xl">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        <div className="flex items-center gap-2">
          <Navigation className="h-4 w-4 text-sky-400" />
          <h3 className="font-mono text-sm font-bold uppercase text-slate-200">OPTIMAL SERPENTINE WAVE PICK ROUTE</h3>
        </div>
      ...
      <div className="relative aspect-[16/7] w-full rounded-lg border border-slate-800 bg-[#080C14] p-4 flex flex-col justify-between overflow-hidden">
          ...
          {sortedItems.map((item, idx) => {
            ...
            return (
              <div key={idx} className="flex flex-col items-center gap-2 relative group shrink-0 px-2">
                <div
                  className={cn(
                    'flex items-center justify-center h-10 w-10 rounded-full border font-mono font-bold text-xs transition-all shadow-lg',
                    isActive
                      ? 'bg-amber-500 text-slate-950 border-amber-300 animate-bounce shadow-[0_0_15px_rgba(245,158,11,0.5)]'
                      : 'bg-slate-900 text-sky-400 border-sky-800'
                  )}
                >
                  {idx + 1}
                </div>
                <span className="text-[11px] font-mono font-bold text-slate-200">{item.bin_code}</span>
                <span className="text-[9px] font-mono text-slate-400 max-w-[90px] truncate text-center">
                  {item.sku}
                </span>

                {/* Connecting Arrow */}
                {idx < sortedItems.length - 1 && (
                  <div className="hidden sm:block absolute left-full top-5 w-8 h-0.5 bg-sky-500/50 -translate-y-1/2 pointer-events-none" />
                )}
              </div>
            );
          })}
